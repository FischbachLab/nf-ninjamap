...
# ninjaMap.sh
#
# Accurate alignment of reads from synthetic microbiome to a combined
...
#
# This script is used to run a single sample on a local server
# No host contamination check is performed unless the human and mouse dbs are installed
...
then
  ...
  aws s3 sync --quiet ${DBPATH}/ ${LOCAL_DB_PATH}/
else
  ...
  tar -xzvf ${LOCAL_DB_PATH}/hCom2_20221117.ninjaIndex.tar.gz  -C ${DOWNLOAD_DB}
  mv ${DOWNLOAD_DB}/HCom2_20221117/db/* ${LOCAL_DB_PATH}/
fi
...
# Get fastq.gz files, only 2 files per sample
...
bowtie2 \
    ...
    -S ${BOWTIE2_OUTPUT}/${SAMPLE_NAME}_overlapped_mapped.sam
