...
# Optional
p.add_argument('-bed', dest='bedfile', action='store', type=str, default='',
                help='masked genomes in bed format.')
p.add_argument('-prefix', dest='prefix', action='store', type=str, default='',
                help='output prefix')
p.add_argument('-threads', dest='threads', action='store', type=int,
                help='number of threads available for this job and subprocesses(use all threads by default)')
...
p.add_argument('-mbq', dest='min_base_qual', action='store', type=int, default=20,
...
p.add_argument('-msv', dest='min_singular_vote', action='store', type=int, default=0, 
                help='minimum singular vote per strain present to consider for real abundance')
p.add_argument('-version', dest='version', action='store_true', default=False, help='NinjaMap version 1.2')
...
#print ninjaMap version number
#ver="1.0"
#if args['version']:
#    print("NinjaMap version "+ ver)
#    exit()
...
perfect_bam =  prefix +'.perfect.bam'
perfect_primary_bam =  prefix +'.perfect_primary.bam'
singular_bam =  prefix +'.singular.bam'
escrow_bam =  prefix +'.escrow.bam'
...
total_reads = set()
if args['debug'] or args['coverage']:
...
#if args['debug']:
    # Write the BAM file for perfect alignment
#perfect_reads = pysam.AlignmentFile(perfect_bam, "wb", template=bamfile)
...
#######################################################################################################
# Convert all reads from singular to escrow if they align to the masked regions
# Open BAM file
# bamfile = pysam.AlignmentFile(bamfile_name, "rb")
#######################################################################################################
masked_reads = defaultdict(int)  
# Parse BED file and fetch overlapping reads
if os.path.isfile(bedfile_name) and os.path.getsize(bedfile_name) > 0:
    with open(bedfile_name) as bed:
        for line in bed:
            if line.startswith("#") or line.strip() == "":
                continue  # Skip comments and empty lines
            contig_name, start, end = line.strip().split()[:3]
            start, end = int(start), int(end)
            #print for debugging
            #print(f"{contig_name}:{start}-{end}")

            for read in bamfile.fetch(contig_name, start, end):
                # save the read_name in a hash table
                read_name, mates_name, read_length, template_length = Reads.extract_read_info(read)
                if read_name in perfect_alignment.keys():
                    masked_reads[read_name] += 1
                #print for debugging
                #print(f"{read_name}\t{read.reference_name}:{read.reference_start}-{read.reference_end}")
logging.info('\tFound %d masked reads.', 
             len(masked_reads))

#for key, value in masked_reads.items():
#    print(f"{key}: {value}")
####################################################################################################
...
if Reads.total_reads_aligned == 0:
        sys.exit("No aligned reads found!")
Reads.reads_w_perfect_alignments = len(perfect_alignment.keys())
if Reads.reads_w_perfect_alignments == 0:
        sys.exit("No perfect aligned reads found!")
...
###############################################################################
# Write the primary_alignment hash or BAM file for perfect primary alignment
###############################################################################
#primary_alignment = defaultdict(list)
#preads = pysam.AlignmentFile(perfect_bam, "rb")
#bamfile = pysam.AlignmentFile(bamfile_name, mode = 'rb')
#perfect_primary_reads = pysam.AlignmentFile(perfect_primary_bam, "wb", template=bamfile)
#for aln in preads.fetch(until_eof=True):
#    if aln.is_proper_pair:
#        read_name, mates_name, read_length, template_length = Reads.extract_read_info(aln)
#        primary_alignment[read_name].append(aln)
        #perfect_primary_reads.write(aln)
#perfect_primary_reads.close()
...
for read_name in perfect_alignment.keys():
    ...
    if read.num_strains == 1 and read_name not in masked_reads:
        # Singular
...
###############################################################################
# Add all masked reads into the escorw bin
for read_key, value in masked_reads.items():
    escrow_read_objects[read_key] = read_objects[read_key]
###############################################################################

#p_bam = pysam.AlignmentFile(perfect_primary_bam, "rb")
if args['coverage']:
    singular_bamfile = pysam.AlignmentFile(singular_bam, "wb", template=bamfile)
    escrow_bamfile = pysam.AlignmentFile(escrow_bam, "wb", template=bamfile)

for name, read in read_objects.items():
    if read.has_voted:
        continue
    
    if name in masked_reads:
        #print(name)
    ...
    if read.mate_has_perfect_match: # and name not in masked_reads:
        # Means both reads in a mate are prefect alignments
        mate = read_objects[read.mates_unique_name]

        if mate.name in masked_reads:
            #print(f"{mate.name} exists!")
            #escrow_read_objects[mate.name] = mate
            #continue
            #exit_early = True
            pass
...
logging.info('\t%d reads for escrow alignment strain abundance out of %d (%7.3f%%) reads with perfect alignments or %7.3f%% of total aligned.',
    ...
    Reads.reads_w_perfect_alignments,
    Reads.total_escrow_reads * 100 / Reads.reads_w_perfect_alignments if Reads.reads_w_perfect_alignments != 0 else 0,
    Reads.total_escrow_reads * 100 / Reads.total_reads_aligned if Reads.total_reads_aligned != 0 else 0)
...
if Reads.total_escrow_reads_discarded > 0:
    logging.info('\t%d out of %d escrow reads had to be discarded because their singular weight was too low or in the masked regions, but %d (%7.3f%%) were still retained',
                Reads.total_escrow_reads_discarded,
                Reads.total_escrow_reads,
                Reads.total_escrow_reads_kept,
                Reads.total_escrow_reads_kept * 100 / Reads.reads_w_perfect_alignments if Reads.reads_w_perfect_alignments != 0 else 0)
