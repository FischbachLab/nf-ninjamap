...
aggregate_read_stats = function(parent_path){
  reads_acct.df = dir_ls(path = parent_path,
    ...
    #mutate(Fragments_Aligned2 = Fragments_Aligned) %>%
...
download_from_s3 = function(local_path, s3_path,debug = FALSE){
  ...
  system(paste("/opt/conda/bin/aws s3 cp",
               s3_path,
               local_path,
               "--recursive --exclude '*' --include '*/ninjaMap/*.ninjaMap.abundance.csv'", sep = " "), ignore.stdout = debug_val, ignore.stderr = debug_val)

  system(paste("/opt/conda/bin/aws s3 cp",
               s3_path,
               local_path,
               "--recursive --exclude '*' --include '*/Stats/read_accounting.csv'", sep = " "), ignore.stdout = debug_val, ignore.stderr = debug_val)

  system(paste("/opt/conda/bin/aws s3 cp",
               s3_path,
               local_path,
               "--recursive --exclude '*' --include '*/ninjaMap/*.ninjaMap.read_stats.csv'", sep = " "), ignore.stdout = debug_val, ignore.stderr = debug_val)
  
  system(paste("/opt/conda/bin/aws s3 cp",
               s3_path,
               local_path,
               "--recursive --exclude '*' --include '*/Logs/*_Contaminants_stats.csv'", sep = " "), ignore.stdout = debug_val, ignore.stderr = debug_val)
}
...
waffle_plot = function(df, title, units = "%", subtitle=NULL){
  ...
  df %>%
    ...
    # added ceiling to show all "squares"
    # because some category( e.g., unaligned rate) is simply to small to show up in the plot
    ggplot(aes(fill=print_name, values=ceiling(values)))   +
    ...
    #theme_ipsum_rc(grid="") + # comment out because it didn't print legends
    #theme_waffle()
