...
then
    s="singular"
    samtools sort ${NINJA_OUTPUT}/${SAMPLE_NAME}.singular.bam > ${NINJA_OUTPUT}/${SAMPLE_NAME}.singular_sorted.bam
    ...
    do
     ...
     s="escrow"
     samtools sort ${NINJA_OUTPUT}/${SAMPLE_NAME}.escrow.bam > ${NINJA_OUTPUT}/${SAMPLE_NAME}.escrow_sorted.bam
...
then
    ...
    samtools sort -n -m ${memPerCore} -@ ${mycpu} "${QC_FASTQ}/Human_bothReadsUnmapped.bam" -o "${QC_FASTQ}/Human_bothReadsUnmapped_sorted.bam"
...
then
  ...
  # sort bam file by read name (-n) to have paired reads next to each other
  samtools sort -n -m ${memPerCore} -@ ${mycpu} "${QC_FASTQ}/Mouse_bothReadsUnmapped.bam" -o "${QC_FASTQ}/Mouse_bothReadsUnmapped_sorted.bam"
